...
interface SelectValueProps {
  placeholder?: string;
  children?: React.ReactNode; // If used to display actual selected value, otherwise taken from context
}
...
export const SelectContent: React.FC<SelectContentProps> = ({ children, className, ...props }) => {
  ...
  return isOpen ? (
    <div
      ref={contentRef}
      className={twMerge(
        "absolute z-50 mt-1 w-full max-h-60 overflow-y-auto rounded-md border bg-popover text-popover-foreground shadow-md outline-none animate-in fade-in-80",
        className
      )}
      {...props}
    >
...
export const SelectItem: React.FC<SelectItemProps> = ({ value, children, className, ...props }) => {
  ...
  const handleClick = () => {
    onValueChange(value);
    setIsOpen(false); // Close select after item is chosen
  };
